...
public:
    AP_Aoa_Backend(AP_Aoa &frontend, uint8_t instance);
    virtual ~AP_Aoa_Backend();
    ...
    // true if sensor reads aoa directly, not via pressure
    virtual bool has_aoa() {return false;}

    // return aoa in m/s if available
    virtual bool get_aoa(float& aoa) {return false;}

    virtual void handle_msp(const MSP::msp_aoa_data_message_t &pkt) {}
#if AP_AOA_EXTERNAL_ENABLED
    virtual void handle_external(const AP_ExternalAHRS::aoa_data_message_t &pkt) {}
#endif
...
protected:
    ...
    bool disable_voltage_correction(void) const {
        return (frontend._options.get() & AP_Aoa::OptionsMask::DISABLE_VOLTAGE_CORRECTION) != 0;
    }

    AP_Aoa::pitot_tube_order get_tube_order(void) const {
#ifndef HAL_BUILD_AP_PERIPH
        return AP_Aoa::pitot_tube_order(frontend.param[instance].tube_order.get());
#else
        return AP_Aoa::pitot_tube_order::PITOT_TUBE_ORDER_AUTO;
#endif
    }
    ...
    float get_aoa_ratio(void) const {
        return frontend.get_aoa_ratio(instance);
    }
