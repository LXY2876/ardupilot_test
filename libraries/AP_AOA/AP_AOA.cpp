...
// #include "AP_Aoa_DroneCAN.h"
// #include "AP_Aoa_NMEA.h"
// #include "AP_Aoa_MSP.h"
// #include "AP_Aoa_External.h"
...
#include <AP_Vehicle/AP_FixedWing.h>
 ...
 #define ARSPD_DEFAULT_TYPE HAL_AOA_TYPE_DEFAULT
 ...
 // connect and find the SIM_DLVR sensors & fault as there is no way to tell them apart
  ...
  #define ARSPD_DEFAULT_TYPE TYPE_I2C_MS4525
      ...
      #define ARSPD_DEFAULT_PIN HAL_DEFAULT_AOA_PIN
  #else
...
const AP_Param::GroupInfo AP_Aoa::var_info[] = {
    ...
    // @Description: Bitmask of options to use with aoa. 0:Disable use based on aoa/groundspeed mismatch (see ARSPD_WIND_MAX), 1:Automatically reenable use based on aoa/groundspeed mismatch recovery (see ARSPD_WIND_MAX) 2:Disable voltage correction, 3:Check that the aoa is statistically consistent with the navigation EKF vehicle and wind velocity estimates using EKF3 (requires AHRS_EKF_TYPE = 3), 4:Report cal offset to GCS
    ...
    // @Description: If the difference between aoa and ground speed is greater than this value the sensor will be marked unhealthy. Using ARSPD_OPTION this health value can be used to disable the sensor.
    // @Description{Copter, Blimp, Rover, Sub}: This parameter and function is not used by this vehicle. Always set to 0.
    // @Units: m/s
    // @User: Advanced
    ...
    // @Param: _WIND_WARN
    // @DisplayName: Aoa and GPS speed difference that gives a warning
    // @Description: If the difference between aoa and GPS speed is greater than this value the sensor will issue a warning. If 0 ARSPD_WIND_MAX is used.
    // @Description{Copter, Blimp, Rover, Sub}: This parameter and function is not used by this vehicle. Always set to 0.
    // @Units: m/s
    // @User: Advanced
    ...
    // @Description: Number of standard deviations applied to the re-enable EKF consistency check that is used when ARSPD_OPTIONS bit position 3 is set. Larger values will make the re-enabling of the aoa sensor faster, but increase the likelihood of re-enabling a degraded sensor. The value can be tuned by using the ARSP.TR log message by setting ARSPD_WIND_GATE to a value that is higher than the value for ARSP.TR observed with a healthy aoa sensor. Occasional transients in ARSP.TR above the value set by ARSPD_WIND_GATE can be tolerated provided they are less than 5 seconds in duration and less than 10% duty cycle.
    ...
    // @Description: The maximum percentage speed change in aoa reports that is allowed due to offset changes between calibrations before a warning is issued. This potential speed error is in percent of ASPD_FBW_MIN. 0 disables. Helps warn of calibrations without pitot being covered.
    // @Range: 0.0 10.0
    // @Units: %
    // @User: Advanced
...
void AP_Aoa::allocate()
{
    ...
    for (uint8_t i=0; i<AOA_MAX_SENSORS; i++) {
        ...
        case TYPE_I2C_MS4525:
            ...
            sensor[i] = NEW_NOTHROW AP_Aoa_MS4525(*this, i);
#endif
            break;
//         case TYPE_SITL:
// #if AP_AOA_SITL_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_SITL(*this, i);
// #endif
//             break;
//         case TYPE_ANALOG:
// #if AP_AOA_ANALOG_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_Analog(*this, i);
// #endif
//             break;
//         case TYPE_I2C_MS5525:
// #if AP_AOA_MS5525_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_MS5525(*this, i, AP_Aoa_MS5525::MS5525_ADDR_AUTO);
// #endif
//             break;
//         case TYPE_I2C_MS5525_ADDRESS_1:
// #if AP_AOA_MS5525_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_MS5525(*this, i, AP_Aoa_MS5525::MS5525_ADDR_1);
// #endif
//             break;
//         case TYPE_I2C_MS5525_ADDRESS_2:
// #if AP_AOA_MS5525_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_MS5525(*this, i, AP_Aoa_MS5525::MS5525_ADDR_2);
// #endif
//             break;
//         case TYPE_I2C_SDP3X:
// #if AP_AOA_SDP3X_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_SDP3X(*this, i);
// #endif
//             break;
//         case TYPE_I2C_DLVR_5IN:
// #if AP_AOA_DLVR_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_DLVR(*this, i, 5);
// #endif
//             break;
//         case TYPE_I2C_DLVR_10IN:
// #if AP_AOA_DLVR_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_DLVR(*this, i, 10);
// #endif
//             break;
//         case TYPE_I2C_DLVR_20IN:
// #if AP_AOA_DLVR_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_DLVR(*this, i, 20);
// #endif
//             break;
//         case TYPE_I2C_DLVR_30IN:
// #if AP_AOA_DLVR_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_DLVR(*this, i, 30);
// #endif
//             break;
//         case TYPE_I2C_DLVR_60IN:
// #if AP_AOA_DLVR_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_DLVR(*this, i, 60);
// #endif  // AP_AOA_DLVR_ENABLED
//             break;
//         case TYPE_I2C_ASP5033:
// #if AP_AOA_ASP5033_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_ASP5033(*this, i);
// #endif
//             break;
//         case TYPE_UAVCAN:
// #if AP_AOA_DRONECAN_ENABLED
//             sensor[i] = AP_Aoa_DroneCAN::probe(*this, i, uint32_t(param[i].bus_id.get()));
// #endif
//             break;
//         case TYPE_NMEA_WATER:
// #if AP_AOA_NMEA_ENABLED
// #if APM_BUILD_TYPE(APM_BUILD_Rover) || APM_BUILD_TYPE(APM_BUILD_ArduSub) 
//             sensor[i] = NEW_NOTHROW AP_Aoa_NMEA(*this, i);
// #endif
// #endif
//             break;
//         case TYPE_MSP:
// #if AP_AOA_MSP_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_MSP(*this, i, 0);
// #endif
//             break;
//         case TYPE_EXTERNAL:
// #if AP_AOA_EXTERNAL_ENABLED
//             sensor[i] = NEW_NOTHROW AP_Aoa_External(*this, i);
// #endif
//             break;
        }
    ...
    for (uint8_t i=0; i<AOA_MAX_SENSORS; i++) {
        if (sensor[i] == nullptr && (enum aoa_type)param[i].type.get() == TYPE_UAVCAN) {
            sensor[i] = AP_Aoa_DroneCAN::probe(*this, i, 0);
            if (sensor[i] != nullptr) {
                num_sensors = i+1;
            }
        }
    }
#endif // AP_AOA_DRONECAN_ENABLED
    ...
    if (AP_HAL::millis() - state[i].cal.start_ms >= 1000 &&
        ...
        if (state[i].cal.count == 0) {
            ...
            if (fixed_wing_parameters != nullptr) {
                // float aoa_min = fixed_wing_parameters->aoa_min.get();
                float aoa_min =0;
                // use percentage of AOA_MIN as criteria for max allowed change in offset
            ...
            if (_options & AP_Aoa::OptionsMask::REPORT_OFFSET ){
                 GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Aoa %u calibrated, offset = %4.0f", i + 1, calibrated_offset);
            } else {
...
void AP_Aoa::read(uint8_t i)
{
    ...
    float aoa_pressure = raw_pressure - get_offset(i);
...
/*
  handle MSP aoa data
 */
void AP_Aoa::handle_msp(const MSP::msp_aoa_data_message_t &pkt)
{
    if (!lib_enabled()) {
        return;
    }
    ...
    if (pkt.instance > 1) {
        return; //supporting 2 aoa sensors at most
    }
...
/*
  handle aoa aoa data
 */
void AP_Aoa::handle_external(const AP_ExternalAHRS::aoa_data_message_t &pkt)
{
    if (!lib_enabled()) {
        return;
    }
