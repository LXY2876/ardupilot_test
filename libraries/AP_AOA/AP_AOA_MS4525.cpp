...
AP_Aoa_MS4525::AP_Aoa_MS4525(AP_Aoa &_frontend, uint8_t _instance) :
    AP_Aoa_Backend(_frontend, _instance)
{
}

// probe for a sensor
bool AP_Aoa_MS4525::probe(uint8_t bus, uint8_t address)
{
...
found_sensor:
    ...
    _dev->register_periodic_callback(20000,
                                     FUNCTOR_BIND_MEMBER(&AP_Aoa_MS4525::_timer, void));
    return true;
}

// start a measurement
void AP_Aoa_MS4525::_measure()
{
...
*/
float AP_Aoa_MS4525::_get_pressure(int16_t dp_raw) const
{
 ...
 */
float AP_Aoa_MS4525::_get_temperature(int16_t dT_raw) const
{
...
void AP_Aoa_MS4525::_collect()
{
    uint8_t data[2];
     
    uint8_t angle_reg_msb=0x0E   ;  
    _measurement_started_ms = 0;
    _dev->transfer(&angle_reg_msb, 1, nullptr, 0);
    ...
    angle_raw = angle_raw & 0x0FFF; // 12-bit angle

    // convert raw value to degrees (0-360)
    float angle = (float)angle_raw * 360.0f / 4096.0f;
 ...
 */
void AP_Aoa_MS4525::_voltage_correction(float &diff_press_pa, float &temperature)
{
...
bool AP_Aoa_MS4525::get_differential_pressure(float &pressure)
{
    WITH_SEMAPHORE(sem);
...
bool AP_Aoa_MS4525::get_temperature(float &temperature)
{
    WITH_SEMAPHORE(sem);
