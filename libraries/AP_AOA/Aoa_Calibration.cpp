/*
 *   auto_calibration.cpp - aoa auto calibration
 *
...
Aoa_Calibration::Aoa_Calibration()
    : P(100,   0,         0,
        0,   100,         0,
        0,     0,  0.000001f)
    , Q0(0.01f)
 ...
 */
void Aoa_Calibration::init(float initial_ratio)
{
    state.z = 1.0f / sqrtf(initial_ratio);
}

/*
  update the state of the aoa calibration - needs to be called
  once a second
 */
float Aoa_Calibration::update(float aoa, const Vector3f &vg, int16_t max_aoa_allowed_during_cal)
{
    ...
    state.x = constrain_float(state.x, -max_aoa_allowed_during_cal, max_aoa_allowed_during_cal);
    state.y = constrain_float(state.y, -max_aoa_allowed_during_cal, max_aoa_allowed_during_cal);
 ...
 */
void AP_Aoa::update_calibration(uint8_t i, const Vector3f &vground, int16_t max_aoa_allowed_during_cal)
{
    ...
    // calculate true aoa, assuming a aoa ratio of 1.0
    float dpress = MAX(get_differential_pressure(i), 0);
    float true_aoa = sqrtf(dpress) * AP::ahrs().get_EAS2TAS();

    float zratio = state[i].calibration.update(true_aoa, vground, max_aoa_allowed_during_cal);
    ...
    if (state[i].counter > 60) {
        if (state[i].last_saved_ratio > 1.05f*param[i].ratio ||
            ...
            state[i].counter = 0;
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Aoa %u ratio reset: %f", i , static_cast<double> (param[i].ratio));
        }
    } else {
        state[i].counter++;
    }
...
/*
  called once a second to do calibration update
 */
void AP_Aoa::update_calibration(const Vector3f &vground, int16_t max_aoa_allowed_during_cal)
{
    for (uint8_t i=0; i<AOA_MAX_SENSORS; i++) {
        update_calibration(i, vground, max_aoa_allowed_during_cal);
    }
    send_aoa_calibration(vground);
}


#if HAL_GCS_ENABLED
void AP_Aoa::send_aoa_calibration(const Vector3f &vground)
{
// #if AP_AOA_AUTOCAL_ENABLE
//     const mavlink_aoa_autocal_t packet{
//         vx: vground.x,
//         vy: vground.y,
//         vz: vground.z,
...
//     };
//     gcs().send_to_active_channels(MAVLINK_MSG_ID_AOA_AUTOCAL,
//                                   (const char *)&packet);
// #endif // AP_AOA_AUTOCAL_ENABLE
}
